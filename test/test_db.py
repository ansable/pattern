    ...
    def test_query(self):
        ...
        for kwargs, sql, rows in (
          ...
          (dict(fields=db.ALL, filters=db.any(("age", 30, "<"), ("name", "john"))),
            ...
            [(1, "john", 30, 2),
             (2, "jack", 20, 2)]),
          (dict(fields=["name", "gender.name"], relations=[db.relation("gender", "id", "gender")], sort=["persons.name"]),
            "select persons.name, gender.name from `persons` left join `gender` on persons.gender=gender.id order by persons.name;",
            [("john", "male"),
             ("jack", "male"),
             ("jane", "female")]),
        ...
        self.assertEqual(v.SQL(),
            "select persons.name, gender.name as gender from `persons` left join `gender` on persons.gender=gender.id  order by persons.name;")
